...
print("問：今天是星期幾？[1-5]")
try:
    weekday: Annotated[int, Interval(ge=1, le=5)] = int(input())
...
print("隨機選中的菜色:")
statement = select(Dish).where(Dish.wday == weekday).order_by(func.random()).limit(num_of_dish)
with Session(engine) as session:
